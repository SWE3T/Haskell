...
happyExpList = Happy_Data_Array.listArray (0,53) ([9264,16,0,960,0,32,32,2048,2048,9264,9264,9264,9264,0,0,0,128,9264,9264,17344,5056,0,9264,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Exp","num","var","'+'","'*'","'&'","'|'","'{'","'='","'}'","let","in","%eof"]
        bit_start = st * 16
        bit_end = (st + 1) * 16
...
action_6 (6) = happyShift action_7
...
happyNewToken action sts stk (tk:tks) =
	...
	TokenAnd -> cont 9;
	TokenOr -> cont 10;
	TokenLBracket -> cont 11;
	TokenAssign -> cont 12;
	TokenRBracket -> cont 13;
	TokenLet -> cont 14;
	TokenIn -> cont 15;
	_ -> happyError' ((tk:tks), [])
	}
