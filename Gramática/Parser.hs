...
happyExpList = Happy_Data_Array.listArray (0,81) ([16864,2056,0,0,63488,16,0,0,0,0,1,64,0,32,8192,0,57346,2113,4216,7682,32900,8455,16864,4104,0,0,0,0,0,0,32768,0,48,16864,8,0,33822,63488,80,1342,16,7680,132,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","Exp","RecordList","num","var","true","false","'+'","'-'","'*'","'&'","'|'","'{'","'='","','","'}'","'.'","let","in","%eof"]
        bit_start = st * 22
        bit_end = (st + 1) * 22
...
happyReduction_12 (HappyTerminal (TokenVar happy_var_3))
	_
	(HappyAbsSyn4  happy_var_1)
	 =  HappyAbsSyn4
